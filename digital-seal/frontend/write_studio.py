...
export default function Studio() {
  ...
  const onDrop = useCallback(fs => {
    const f = fs[0];
    if (f) { setFile(f); setPreview(URL.createObjectURL(f)); setStep(0); setPct(0); setSeal(null); setTxHash(""); }
  }, []);
        ...
        <div style={C.col}>
          ...
          <div style={C.card}>
            ...
            <div style={{ marginTop:20,paddingTop:16,borderTop:"1px solid rgba(255,255,255,.06)" }}>
              <div style={C.row}><span style={{ color:"#64748b" }}>Phí gas ước tính</span><span>~0.005 ETH</span></div>
              <div style={C.row}><span style={{ color:"#64748b" }}>Phí dịch vụ</span><span style={{ color:"#4ade80",fontWeight:600 }}>Miễn phí</span></div>
              <div style={{ ...C.row,borderBottom:"none",fontWeight:700,fontSize:14,marginTop:4 }}>
                <span>Tổng cộng</span><span style={{ color:"#818cf8" }}>~$12.50</span>
              </div>
            </div>
          </div>
...
with open(r'd:\Block_chain\doanblockchain\digital-seal\frontend\src\pages\Studio.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Done")
